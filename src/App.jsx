...
const firebaseConfig = {
  ...
  storageBucket: "robot-tercih-dkab.firebasestorage.app",
...
// Deterministik RNG + shuffle (seed’e göre aynı sonuç)
...
// Yerleştirme: önce 40 tercihe göre, kalanlar boş kontenjanlara “rastgele” (seed ile)
function simulateAssignments(teachers, schools, seed) {
  const remain = new Map();
  schools.forEach((s) => remain.set(s.id, Number(s.quota) || 0));
  const sorted = [...teachers].sort((a, b) => (a.rank - b.rank) || a.name.localeCompare(b.name, "tr"));
  const out = [];
  for (const t of sorted) {
    let placed = null;
    const prefs = Array.isArray(t.preferences) ? t.preferences.slice(0, 40) : [];
    for (let i = 0; i < prefs.length; i++) {
      const sid = prefs[i];
      const r = remain.get(sid) ?? 0;
      if (r > 0) {
        remain.set(sid, r - 1);
        placed = { teacherId: t.id, schoolId: sid, method: "preference", matchIndex: i };
        break;
      }
    }
    if (!placed) out.push({ teacherId: t.id, schoolId: null, method: "none", matchIndex: null });
    else out.push(placed);
  }
  const seats = [];
  for (const s of schools) { const r = remain.get(s.id) ?? 0; for (let i = 0; i < r; i++) seats.push(s.id); }
  seededShuffle(seats, seed);
  for (let i = 0; i < out.length; i++) {
    const a = out[i];
    if (a.method === "none" && a.schoolId === null) {
      const seat = seats.pop();
      if (seat) out[i] = { ...a, schoolId: seat, method: "random" };
    }
  }
  return out;
}
...
export default function App() {
  ...
  const [teachers, setTeachers] = useState([]); // {id,name,rank,preferences[],ownerUid}
  const [schools, setSchools] = useState([]);   // {id,name,province,quota}
  ...
  // Admin rolleri (settings/roles: { admins: [uid, ...] })
  useEffect(() => {
    if (!user) { setIsAdmin(false); return; }
    const unsub = onSnapshot(doc(db, "settings", "roles"), (d) => {
      const v = d.data() || {};
      const admins = Array.isArray(v.admins) ? v.admins : [];
  ...
  // Kullanıcı başına tek öğretmen kaydı
  ...
  return (
    <div style={{ padding: 16, maxWidth: 1000, margin: "0 auto", fontFamily: "system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial" }}>
      ...
      <section style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
        ...
        />
        <button onClick={() => setGlobalSeed(Math.floor(Date.now() % 100000))} disabled={!isAdmin} style={{ padding: "6px 10px", background: "#e5e7eb", borderRadius: 8 }}>
          Rastgele Seed
        </button>
      ...
      {/* Öğretmen bölümü */}
      {!myTeacher ? (
        <NewTeacherForm onCreate={addMyTeacher} />
      ) : (
        <PreferenceEditor teacher={myTeacher} schools={schools} onChange={updateMyPrefs} />
      )}
          ...
          <table style={{ width: "100%", fontSize: 14 }}>
            ...
            <tbody>
              {[...teachers].sort((a, b) => (a.rank - b.rank) || a.name.localeCompare(b.name, "tr")).map((t) => {
                ...
                const sch = a?.schoolId ? schoolMap.get(a.schoolId) : null;
                return (
                  <tr key={t.id} style={{ borderTop: "1px solid #e5e7eb" }}>
                    ...
                    {isAdmin && (
                      <td style={{ padding: 8, textAlign: "right" }}>
                        <button onClick={() => deleteTeacher(t.id)} style={{ padding: "4px 8px", fontSize: 12, background: "#fee2e2", color: "#991b1b", borderRadius: 6 }}>
                          Sil
                        </button>
                      </td>
                    )}
  ...
  return (
    <section style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: 12 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ad Soyad" style={{ padding: "8px 10px", border: "1px solid #d1d5db", borderRadius: 6, flex: "1 1 220px" }} />
        <input value={rank} onChange={(e) => setRank(e.target.value)} placeholder="Sıra (örn. 1)" style={{ padding: "8px 10px", border: "1px solid #d1d5db", borderRadius: 6, width: 140 }} />
        <button onClick={() => onCreate(name, Number(rank))} style={{ padding: "8px 12px", background: "#4f46e5", color: "white", borderRadius: 8 }}>Ekle</button>
      </div>
...
function PreferenceEditor({ teacher, schools, onChange }) {
  ...
  const add = () => {
    if (sel && !teacher.preferences.includes(sel)) onChange([...(teacher.preferences || []), sel]);
    setSel("");
  };
  const remove = (sid) => {
    onChange((teacher.preferences || []).filter(p => p !== sid));
  };
  const up = (idx) => {
  ...
  const clearAll = () => {
    if (confirm("Tüm tercihleri temizlemek istiyor musunuz?")) onChange([]);
  };
  ...
  return (
    <section style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: 12 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr auto", gap: 8 }}>
        ...
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Okul ara" style={{ padding: "8px 10px", border: "1px solid #d1d5db", borderRadius: 6 }} />
